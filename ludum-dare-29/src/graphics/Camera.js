...
function Camera(zones, window, glCanvasWidth, glCanvasHeight) {
	...
	this.camera = new THREE.PerspectiveCamera(45, glCanvasWidth / glCanvasHeight, 0.1, 10000.0);
	// 0.1, 10
...
Camera.prototype.getBoundingBox = function() {
	...
	{
		this.boundingBox = new BoundingBox(this.yawObject.position.x, this.width, this.yawObject.position.y, this.height, this.yawObject.position.z, this.depth);
	}
...
Camera.prototype.isValidMove = function(movementX, movementZ) {
	...
	if (movementX !== 0)
	{
		...
		if (this.debug)
			console.log("Camera: movementX: " + movementX);
	}
	if (movementZ !== 0)
	{
		clonedYawObject.translateZ(movementZ);
		if (this.debug)
			console.log("Camera: movementZ: " + movementZ);
	}

	if (this.debug)
	{
		console.log("Camera: clonedYawObject (x,y,z): " + clonedYawObject.position.x + "," + clonedYawObject.position.y + "," + clonedYawObject.position.z);
	}

	var newBoundingBox = new BoundingBox(clonedYawObject.position.x, this.width, clonedYawObject.position.y, this.height, clonedYawObject.position.z, this.depth);
	...
	for (var i = 0; i < touchedZones.length; ++i)
	{
		...
		{
			if (this.debug)
				console.log("Camera: isValidMove: false");
	...
	if (this.debug)
		console.log("Camera: isValidMove: true");
